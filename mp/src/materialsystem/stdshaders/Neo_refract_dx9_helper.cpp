...
void InitParamsRefract_DX9( CBaseVSShader *pShader, IMaterialVar** params, const char *pMaterialName, Neo_Refract_DX9_Vars_t &info )
{
	...
	if (!params[info.m_nAlpha]->IsDefined())
	{
		params[info.m_nAlpha]->SetIntValue(MAX_ALPHA);
	}

	SET_FLAGS2( MATERIAL_VAR2_NEEDS_POWER_OF_TWO_FRAME_BUFFER_TEXTURE );
}

void InitRefract_DX9( CBaseVSShader *pShader, IMaterialVar** params, Neo_Refract_DX9_Vars_t &info )
{
...
void DrawRefract_DX9( CBaseVSShader *pShader, IMaterialVar** params, IShaderDynamicAPI *pShaderAPI,
	IShaderShadow* pShaderShadow, Neo_Refract_DX9_Vars_t &info, VertexCompressionType_t vertexCompression)
{
	...
	int nAlpha = params[info.m_nAlpha]->GetIntValue();
	if (nAlpha < 0) {
		nAlpha = 0;
	}
	else if (nAlpha > MAX_ALPHA) {
		nAlpha = MAX_ALPHA;
	}
	...
	SHADOW_STATE
	{
		...
		DECLARE_STATIC_VERTEX_SHADER( neo_refract_vs20 );
		SET_STATIC_VERTEX_SHADER_COMBO( MODEL,  bIsModel );
		SET_STATIC_VERTEX_SHADER_COMBO( COLORMODULATE, bColorModulate );
		SET_STATIC_VERTEX_SHADER( neo_refract_vs20 );
		...
		{
			DECLARE_STATIC_PIXEL_SHADER( neo_refract_ps20b );
			...
			SET_STATIC_PIXEL_SHADER_COMBO(ALPHA, nAlpha);

			SET_STATIC_PIXEL_SHADER( neo_refract_ps20b );
		}
		else
		{
			DECLARE_STATIC_PIXEL_SHADER( neo_refract_ps20 );
			SET_STATIC_PIXEL_SHADER_COMBO( BLUR,  blurAmount );
			SET_STATIC_PIXEL_SHADER_COMBO( FADEOUTONSILHOUETTE,  bFadeOutOnSilhouette );
			SET_STATIC_PIXEL_SHADER_COMBO( CUBEMAP,  bHasEnvmap );
			SET_STATIC_PIXEL_SHADER_COMBO( REFRACTTINTTEXTURE,  bRefractTintTexture );
			...
			SET_STATIC_PIXEL_SHADER( neo_refract_ps20 );
		}
		pShader->DefaultFog();
		if( bMasked )
		{
	...
	DYNAMIC_STATE
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER( neo_refract_vs20 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING,  pShaderAPI->GetCurrentNumBones() > 0 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
		SET_DYNAMIC_VERTEX_SHADER( neo_refract_vs20 );
		...
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(neo_refract_ps20b);
			...
			SET_DYNAMIC_PIXEL_SHADER(neo_refract_ps20b);
		}
		else
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(neo_refract_ps20);
			SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
			SET_DYNAMIC_PIXEL_SHADER(neo_refract_ps20);
		}
